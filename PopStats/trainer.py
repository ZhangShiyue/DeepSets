...
class Trainer(object):
    ...
    def fit(self, train, valid=None):
        ...
        for j in trange(self.num_epochs, desc="Epochs: ", ncols=80):
            ...
            for X, y in train_iterator:
                ...
                loss = self.l2(y_pred, Variable(torch.from_numpy(np.reshape(y, [len(y), 1])).cuda()))
    ...
    def evaluate(self, test):
        ...
        for X, y in test_iterator:
            counts += 1
            y_pred = self.model(Variable(torch.from_numpy(X)).cuda())
            sum_mae += self.l1(y_pred, Variable(torch.from_numpy(np.reshape(y, [len(y), 1]))).cuda()).data.cpu().numpy()
            sum_mse += self.l2(y_pred, Variable(torch.from_numpy(np.reshape(y, [len(y), 1]))).cuda()).data.cpu().numpy()
